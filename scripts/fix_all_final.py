#!/usr/bin/env python3
"""综合修复：修复 EdgeLite 设备配置中的 slave_id、AB 协议名、MQTT broker、OPC UA NodeId。"""
import json
import time
...
# 检查 EdgeLite 支持的协议列表
print("=== 0. 检查 EdgeLite 支持的协议 ===")
code, data = el.get("/drivers/protocols")
if code == 200 and data:
    d = data.get("data", data) if isinstance(data, dict) else data
    protocols = d.get("protocols", []) if isinstance(d, dict) else d
    print(f"  支持的协议: {json.dumps(protocols, ensure_ascii=False)[:300]}")

# 重新创建所有 EdgeLite 设备（修正所有配置问题）
print("\n=== 1. 重新创建所有 EdgeLite 设备 ===")
...
for dev_id, config in DEVICE_CONFIGS.items():
    ...
    if code == 201 or code == 200:
        print(f"  创建 {dev_id}: HTTP {code} OK")
    else:
        print(f"  创建 {dev_id}: HTTP {code}: {json.dumps(data, ensure_ascii=False)[:300] if data else ''}")
    time.sleep(1)
