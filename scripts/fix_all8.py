#!/usr/bin/env python3
"""修复脚本8：删除并重新创建 EdgeLite S7/FINS/MQTT/OPC UA 设备。"""
import json
import time
...
# ===================================================================
# 1. 删除并重新创建 EdgeLite 设备
# ===================================================================
print("=== 1. 删除并重新创建 EdgeLite 设备 ===")
for dev_id in ["pf-s7", "pf-fins", "pf-mqtt", "pf-opcua"]:
    ...
    # 重新创建
    config = DEVICE_CONFIGS[dev_id]
    code, data = el.post("/devices", config)
    print(f"  创建 {dev_id}: HTTP {code}: {json.dumps(data, ensure_ascii=False)[:200] if data else ''}")
    time.sleep(1)
...
for dev_id in ["pf-s7", "pf-fins", "pf-mqtt", "pf-opcua"]:
    ...
    if code == 200 and data:
        ...
        print(f"  {dev_id}: connected={d.get('is_connected')} total_reads={d.get('total_reads')} failures={d.get('consecutive_failures')}")
    else:
        print(f"  {dev_id}: HTTP {code}")
