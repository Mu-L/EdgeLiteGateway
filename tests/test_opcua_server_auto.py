...
class TestAutoSmoke:
    ...
    def test_update_point_value_callable(self):
        ...
        assert obj is not None, (
            f'{_MODULE} 中解析不到 update_point_value：模块属性与所有类属性表均未命中，'
            f'请确认 API 是否已删除或改名')
        assert callable(obj) or isinstance(obj, property), (
            f'{_MODULE}.update_point_value 不可调用 (owner={owner!r})')
    ...
    def test_subscribe_callable(self):
        ...
        assert obj is not None, (
            f'{_MODULE} 中解析不到 subscribe：模块属性与所有类属性表均未命中，'
            f'请确认 API 是否已删除或改名')
        assert callable(obj) or isinstance(obj, property), (
            f'{_MODULE}.subscribe 不可调用 (owner={owner!r})')

    def test_unsubscribe_callable(self):
        owner, obj = _auto_resolve('unsubscribe')
        assert obj is not None, (
            f'{_MODULE} 中解析不到 unsubscribe：模块属性与所有类属性表均未命中，'
            f'请确认 API 是否已删除或改名')
        assert callable(obj) or isinstance(obj, property), (
            f'{_MODULE}.unsubscribe 不可调用 (owner={owner!r})')

    def test_get_nodes_callable(self):
        owner, obj = _auto_resolve('get_nodes')
        assert obj is not None, (
            f'{_MODULE} 中解析不到 get_nodes：模块属性与所有类属性表均未命中，'
            f'请确认 API 是否已删除或改名')
        assert callable(obj) or isinstance(obj, property), (
            f'{_MODULE}.get_nodes 不可调用 (owner={owner!r})')

    def test_get_subscriptions_callable(self):
        owner, obj = _auto_resolve('get_subscriptions')
        assert obj is not None, (
            f'{_MODULE} 中解析不到 get_subscriptions：模块属性与所有类属性表均未命中，'
            f'请确认 API 是否已删除或改名')
        assert callable(obj) or isinstance(obj, property), (
            f'{_MODULE}.get_subscriptions 不可调用 (owner={owner!r})')
    ...
    def test_get_user_callable(self):
        ...
        assert obj is not None, (
            f'{_MODULE} 中解析不到 get_user：模块属性与所有类属性表均未命中，'
            f'请确认 API 是否已删除或改名')
        assert callable(obj) or isinstance(obj, property), (
            f'{_MODULE}.get_user 不可调用 (owner={owner!r})')
