...
class TestAutoSmoke:
    ...
    def test_request_state_callable(self):
        ...
        assert obj is not None, (
            f'{_MODULE} 中解析不到 request_state：模块属性与所有类属性表均未命中，'
            f'请确认 API 是否已删除或改名')
        assert callable(obj) or isinstance(obj, property), (
            f'{_MODULE}.request_state 不可调用 (owner={owner!r})')

    def test_send_process_data_callable(self):
        owner, obj = _auto_resolve('send_process_data')
        assert obj is not None, (
            f'{_MODULE} 中解析不到 send_process_data：模块属性与所有类属性表均未命中，'
            f'请确认 API 是否已删除或改名')
        assert callable(obj) or isinstance(obj, property), (
            f'{_MODULE}.send_process_data 不可调用 (owner={owner!r})')

    def test_receive_process_data_callable(self):
        owner, obj = _auto_resolve('receive_process_data')
        assert obj is not None, (
            f'{_MODULE} 中解析不到 receive_process_data：模块属性与所有类属性表均未命中，'
            f'请确认 API 是否已删除或改名')
        assert callable(obj) or isinstance(obj, property), (
            f'{_MODULE}.receive_process_data 不可调用 (owner={owner!r})')
    ...
    def test_slaves_callable(self):
        ...
        assert obj is not None, (
            f'{_MODULE} 中解析不到 slaves：模块属性与所有类属性表均未命中，'
            f'请确认 API 是否已删除或改名')
        assert callable(obj) or isinstance(obj, property), (
            f'{_MODULE}.slaves 不可调用 (owner={owner!r})')
    ...
    def test_get_state_name_callable(self):
        ...
        assert obj is not None, (
            f'{_MODULE} 中解析不到 get_state_name：模块属性与所有类属性表均未命中，'
            f'请确认 API 是否已删除或改名')
        assert callable(obj) or isinstance(obj, property), (
            f'{_MODULE}.get_state_name 不可调用 (owner={owner!r})')

    def test_parse_vendor_product_callable(self):
        owner, obj = _auto_resolve('parse_vendor_product')
        assert obj is not None, (
            f'{_MODULE} 中解析不到 parse_vendor_product：模块属性与所有类属性表均未命中，'
            f'请确认 API 是否已删除或改名')
        assert callable(obj) or isinstance(obj, property), (
            f'{_MODULE}.parse_vendor_product 不可调用 (owner={owner!r})')
