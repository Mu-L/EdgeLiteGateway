...
class TestAutoSmoke:
    ...
    def test_read_status_callable(self):
        ...
        assert obj is not None, (
            f"{_MODULE} 中解析不到 read_status：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.read_status 不可调用 (owner={owner!r})"

    def test_read_position_callable(self):
        owner, obj = _auto_resolve("read_position")
        assert obj is not None, (
            f"{_MODULE} 中解析不到 read_position：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.read_position 不可调用 (owner={owner!r})"

    def test_read_program_number_callable(self):
        owner, obj = _auto_resolve("read_program_number")
        assert obj is not None, (
            f"{_MODULE} 中解析不到 read_program_number：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.read_program_number 不可调用 (owner={owner!r})"

    def test_read_feedrate_callable(self):
        owner, obj = _auto_resolve("read_feedrate")
        assert obj is not None, (
            f"{_MODULE} 中解析不到 read_feedrate：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.read_feedrate 不可调用 (owner={owner!r})"

    def test_read_spindle_speed_callable(self):
        owner, obj = _auto_resolve("read_spindle_speed")
        assert obj is not None, (
            f"{_MODULE} 中解析不到 read_spindle_speed：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.read_spindle_speed 不可调用 (owner={owner!r})"

    def test_read_alarms_callable(self):
        owner, obj = _auto_resolve("read_alarms")
        assert obj is not None, (
            f"{_MODULE} 中解析不到 read_alarms：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.read_alarms 不可调用 (owner={owner!r})"
