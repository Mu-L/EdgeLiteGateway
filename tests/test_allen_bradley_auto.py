...
class TestAutoSmoke:
    ...
    def test_get_cip_error_dist_callable(self):
        ...
        assert obj is not None, (
            f'{_MODULE} 中解析不到 get_cip_error_dist：模块属性与所有类属性表均未命中，'
            f'请确认 API 是否已删除或改名')
        assert callable(obj) or isinstance(obj, property), (
            f'{_MODULE}.get_cip_error_dist 不可调用 (owner={owner!r})')

    def test_reload_rules_callable(self):
        owner, obj = _auto_resolve('reload_rules')
        assert obj is not None, (
            f'{_MODULE} 中解析不到 reload_rules：模块属性与所有类属性表均未命中，'
            f'请确认 API 是否已删除或改名')
        assert callable(obj) or isinstance(obj, property), (
            f'{_MODULE}.reload_rules 不可调用 (owner={owner!r})')
    ...
    def test_discover_tags_callable(self):
        ...
        assert obj is not None, (
            f'{_MODULE} 中解析不到 discover_tags：模块属性与所有类属性表均未命中，'
            f'请确认 API 是否已删除或改名')
        assert callable(obj) or isinstance(obj, property), (
            f'{_MODULE}.discover_tags 不可调用 (owner={owner!r})')

    def test_browse_struct_members_callable(self):
        owner, obj = _auto_resolve('browse_struct_members')
        assert obj is not None, (
            f'{_MODULE} 中解析不到 browse_struct_members：模块属性与所有类属性表均未命中，'
            f'请确认 API 是否已删除或改名')
        assert callable(obj) or isinstance(obj, property), (
            f'{_MODULE}.browse_struct_members 不可调用 (owner={owner!r})')

    def test_browse_array_range_callable(self):
        owner, obj = _auto_resolve('browse_array_range')
        assert obj is not None, (
            f'{_MODULE} 中解析不到 browse_array_range：模块属性与所有类属性表均未命中，'
            f'请确认 API 是否已删除或改名')
        assert callable(obj) or isinstance(obj, property), (
            f'{_MODULE}.browse_array_range 不可调用 (owner={owner!r})')
