...
class TestAutoSmoke:
    ...
    def test_log_callable(self):
        ...
        assert obj is not None, f"{_MODULE} 中解析不到 log：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.log 不可调用 (owner={owner!r})"

    def test_log_cert_switch_callable(self):
        owner, obj = _auto_resolve("log_cert_switch")
        assert obj is not None, (
            f"{_MODULE} 中解析不到 log_cert_switch：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.log_cert_switch 不可调用 (owner={owner!r})"
    ...
    def test_log_rbac_check_callable(self):
        ...
        assert obj is not None, (
            f"{_MODULE} 中解析不到 log_rbac_check：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.log_rbac_check 不可调用 (owner={owner!r})"

    def test_log_config_version_callable(self):
        owner, obj = _auto_resolve("log_config_version")
        assert obj is not None, (
            f"{_MODULE} 中解析不到 log_config_version：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.log_config_version 不可调用 (owner={owner!r})"

    def test_log_ota_callable(self):
        owner, obj = _auto_resolve("log_ota")
        assert obj is not None, (
            f"{_MODULE} 中解析不到 log_ota：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.log_ota 不可调用 (owner={owner!r})"
