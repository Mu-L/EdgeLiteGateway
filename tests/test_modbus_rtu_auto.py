...
class TestAutoSmoke:
    ...
    def test_get_polling_interval_callable(self):
        ...
        assert obj is not None, (
            f"{_MODULE} 中解析不到 get_polling_interval：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.get_polling_interval 不可调用 (owner={owner!r})"
    ...
    def test_rollback_edge_rule_callable(self):
        ...
        assert obj is not None, (
            f"{_MODULE} 中解析不到 rollback_edge_rule：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.rollback_edge_rule 不可调用 (owner={owner!r})"

    def test_get_edge_rule_versions_callable(self):
        owner, obj = _auto_resolve("get_edge_rule_versions")
        assert obj is not None, (
            f"{_MODULE} 中解析不到 get_edge_rule_versions：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), (
    ...
    def test_get_edge_active_alarms_callable(self):
        ...
        assert obj is not None, (
            f"{_MODULE} 中解析不到 get_edge_active_alarms：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), (
    ...
    def test_get_edge_stats_callable(self):
        ...
        assert obj is not None, (
            f"{_MODULE} 中解析不到 get_edge_stats：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.get_edge_stats 不可调用 (owner={owner!r})"

    def test_reload_edge_rules_callable(self):
        owner, obj = _auto_resolve("reload_edge_rules")
        assert obj is not None, (
            f"{_MODULE} 中解析不到 reload_edge_rules：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.reload_edge_rules 不可调用 (owner={owner!r})"

    def test_set_network_status_callable(self):
        owner, obj = _auto_resolve("set_network_status")
        assert obj is not None, (
            f"{_MODULE} 中解析不到 set_network_status：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.set_network_status 不可调用 (owner={owner!r})"
    ...
    def test_force_sync_all_callable(self):
        ...
        assert obj is not None, (
            f"{_MODULE} 中解析不到 force_sync_all：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.force_sync_all 不可调用 (owner={owner!r})"

    def test_sync_sqlite_to_upload_callable(self):
        owner, obj = _auto_resolve("sync_sqlite_to_upload")
        assert obj is not None, (
            f"{_MODULE} 中解析不到 sync_sqlite_to_upload：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.sync_sqlite_to_upload 不可调用 (owner={owner!r})"

    def test_get_persist_stats_callable(self):
        owner, obj = _auto_resolve("get_persist_stats")
        assert obj is not None, (
            f"{_MODULE} 中解析不到 get_persist_stats：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.get_persist_stats 不可调用 (owner={owner!r})"
    ...
    def test_update_device_config_callable(self):
        ...
        assert obj is not None, (
            f"{_MODULE} 中解析不到 update_device_config：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.update_device_config 不可调用 (owner={owner!r})"

    def test_rollback_device_config_callable(self):
        owner, obj = _auto_resolve("rollback_device_config")
        assert obj is not None, (
            f"{_MODULE} 中解析不到 rollback_device_config：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), (
    ...
    def test_list_config_versions_callable(self):
        ...
        assert obj is not None, (
            f"{_MODULE} 中解析不到 list_config_versions：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.list_config_versions 不可调用 (owner={owner!r})"

    def test_get_config_version_callable(self):
        owner, obj = _auto_resolve("get_config_version")
        assert obj is not None, (
            f"{_MODULE} 中解析不到 get_config_version：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.get_config_version 不可调用 (owner={owner!r})"
    ...
    def test_export_config_json_callable(self):
        ...
        assert obj is not None, (
            f"{_MODULE} 中解析不到 export_config_json：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.export_config_json 不可调用 (owner={owner!r})"

    def test_import_config_json_callable(self):
        owner, obj = _auto_resolve("import_config_json")
        assert obj is not None, (
            f"{_MODULE} 中解析不到 import_config_json：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.import_config_json 不可调用 (owner={owner!r})"

    def test_verify_config_integrity_callable(self):
        owner, obj = _auto_resolve("verify_config_integrity")
        assert obj is not None, (
            f"{_MODULE} 中解析不到 verify_config_integrity：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), (
    ...
    def test_audit_write_point_callable(self):
        ...
        assert obj is not None, (
            f"{_MODULE} 中解析不到 audit_write_point：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.audit_write_point 不可调用 (owner={owner!r})"

    def test_audit_failover_callable(self):
        owner, obj = _auto_resolve("audit_failover")
        assert obj is not None, (
            f"{_MODULE} 中解析不到 audit_failover：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.audit_failover 不可调用 (owner={owner!r})"
