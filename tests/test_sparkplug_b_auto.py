"""自动生成冒烟测试 - src.edgelite.drivers.sparkplug_b
...
_MODULE = "src.edgelite.drivers.sparkplug_b"
...
class TestAutoSmoke:
    ...
    def test_handle_point_update_callable(self):
        owner, obj = _auto_resolve("handle_point_update")
        assert obj is not None, (
            f"{_MODULE} 中解析不到 handle_point_update：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.handle_point_update 不可调用 (owner={owner!r})"
