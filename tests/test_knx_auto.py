...
class TestAutoSmoke:
    ...
    def test_read_group_value_callable(self):
        ...
        assert obj is not None, (
            f"{_MODULE} 中解析不到 read_group_value：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.read_group_value 不可调用 (owner={owner!r})"

    def test_write_group_value_callable(self):
        owner, obj = _auto_resolve("write_group_value")
        assert obj is not None, (
            f"{_MODULE} 中解析不到 write_group_value：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.write_group_value 不可调用 (owner={owner!r})"

    def test_handle_packet_callable(self):
        owner, obj = _auto_resolve("handle_packet")
        assert obj is not None, (
            f"{_MODULE} 中解析不到 handle_packet：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.handle_packet 不可调用 (owner={owner!r})"
    ...
    def test_get_latest_value_callable(self):
        ...
        assert obj is not None, (
            f"{_MODULE} 中解析不到 get_latest_value：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.get_latest_value 不可调用 (owner={owner!r})"
    ...
    def test_get_event_status_callable(self):
        ...
        assert obj is not None, (
            f"{_MODULE} 中解析不到 get_event_status：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.get_event_status 不可调用 (owner={owner!r})"
