...
class TestAutoSmoke:
    ...
    def test_get_callable(self):
        ...
        assert obj is not None, f"{_MODULE} 中解析不到 get：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.get 不可调用 (owner={owner!r})"

    def test_set_callable(self):
        owner, obj = _auto_resolve("set")
        assert obj is not None, f"{_MODULE} 中解析不到 set：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.set 不可调用 (owner={owner!r})"

    def test_pop_callable(self):
        owner, obj = _auto_resolve("pop")
        assert obj is not None, f"{_MODULE} 中解析不到 pop：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.pop 不可调用 (owner={owner!r})"
    ...
    def test_keys_callable(self):
        ...
        assert obj is not None, (
            f"{_MODULE} 中解析不到 keys：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.keys 不可调用 (owner={owner!r})"

    def test_read_error_rate_callable(self):
        owner, obj = _auto_resolve("read_error_rate")
        assert obj is not None, (
            f"{_MODULE} 中解析不到 read_error_rate：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.read_error_rate 不可调用 (owner={owner!r})"

    def test_write_error_rate_callable(self):
        owner, obj = _auto_resolve("write_error_rate")
        assert obj is not None, (
            f"{_MODULE} 中解析不到 write_error_rate：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.write_error_rate 不可调用 (owner={owner!r})"

    def test_p95_latency_ms_callable(self):
        owner, obj = _auto_resolve("p95_latency_ms")
        assert obj is not None, (
            f"{_MODULE} 中解析不到 p95_latency_ms：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.p95_latency_ms 不可调用 (owner={owner!r})"

    def test_is_healthy_callable(self):
        owner, obj = _auto_resolve("is_healthy")
        assert obj is not None, (
            f"{_MODULE} 中解析不到 is_healthy：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.is_healthy 不可调用 (owner={owner!r})"

    def test_health_score_callable(self):
        owner, obj = _auto_resolve("health_score")
        assert obj is not None, (
            f"{_MODULE} 中解析不到 health_score：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.health_score 不可调用 (owner={owner!r})"

    def test_effective_state_callable(self):
        owner, obj = _auto_resolve("effective_state")
        assert obj is not None, (
            f"{_MODULE} 中解析不到 effective_state：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.effective_state 不可调用 (owner={owner!r})"
    ...
    def test_is_running_callable(self):
        ...
        assert obj is not None, (
            f"{_MODULE} 中解析不到 is_running：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.is_running 不可调用 (owner={owner!r})"
    ...
    def test_get_all_health_stats_callable(self):
        ...
        assert obj is not None, (
            f"{_MODULE} 中解析不到 get_all_health_stats：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.get_all_health_stats 不可调用 (owner={owner!r})"

    def test_reset_health_stats_callable(self):
        owner, obj = _auto_resolve("reset_health_stats")
        assert obj is not None, (
            f"{_MODULE} 中解析不到 reset_health_stats：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.reset_health_stats 不可调用 (owner={owner!r})"

    def test_get_connection_quality_callable(self):
        owner, obj = _auto_resolve("get_connection_quality")
        assert obj is not None, (
            f"{_MODULE} 中解析不到 get_connection_quality：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
    ...
    def test_reconnect_callable(self):
        ...
        assert obj is not None, (
            f"{_MODULE} 中解析不到 reconnect：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.reconnect 不可调用 (owner={owner!r})"
    ...
    def test_reconnect_with_backoff_callable(self):
        ...
        assert obj is not None, (
            f"{_MODULE} 中解析不到 reconnect_with_backoff：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), (
    ...
    def test_get_capabilities_callable(self):
        ...
        assert obj is not None, (
            f"{_MODULE} 中解析不到 get_capabilities：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.get_capabilities 不可调用 (owner={owner!r})"

    def test_get_connection_status_callable(self):
        owner, obj = _auto_resolve("get_connection_status")
        assert obj is not None, (
            f"{_MODULE} 中解析不到 get_connection_status：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.get_connection_status 不可调用 (owner={owner!r})"

    def test_validate_config_callable(self):
        owner, obj = _auto_resolve("validate_config")
        assert obj is not None, (
            f"{_MODULE} 中解析不到 validate_config：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.validate_config 不可调用 (owner={owner!r})"

    def test_get_write_policy_callable(self):
        owner, obj = _auto_resolve("get_write_policy")
        assert obj is not None, (
            f"{_MODULE} 中解析不到 get_write_policy：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.get_write_policy 不可调用 (owner={owner!r})"

    def test_check_write_allowed_callable(self):
        owner, obj = _auto_resolve("check_write_allowed")
        assert obj is not None, (
            f"{_MODULE} 中解析不到 check_write_allowed：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.check_write_allowed 不可调用 (owner={owner!r})"
    ...
    def test_map_exception_callable(self):
        ...
        assert obj is not None, (
            f"{_MODULE} 中解析不到 map_exception：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.map_exception 不可调用 (owner={owner!r})"
