...
class TestAutoSmoke:
    ...
    def test_set_on_action_callback_callable(self):
        ...
        assert obj is not None, (
            f"{_MODULE} 中解析不到 set_on_action_callback：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
    ...
    def test_add_rule_callable(self):
        ...
        assert obj is not None, (
            f"{_MODULE} 中解析不到 add_rule：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.add_rule 不可调用 (owner={owner!r})"

    def test_get_rule_callable(self):
        owner, obj = _auto_resolve("get_rule")
        assert obj is not None, (
            f"{_MODULE} 中解析不到 get_rule：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.get_rule 不可调用 (owner={owner!r})"

    def test_get_rules_for_device_callable(self):
        owner, obj = _auto_resolve("get_rules_for_device")
        assert obj is not None, (
            f"{_MODULE} 中解析不到 get_rules_for_device：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.get_rules_for_device 不可调用 (owner={owner!r})"

    def test_get_all_rules_callable(self):
        owner, obj = _auto_resolve("get_all_rules")
        assert obj is not None, (
            f"{_MODULE} 中解析不到 get_all_rules：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.get_all_rules 不可调用 (owner={owner!r})"

    def test_evaluate_point_callable(self):
        owner, obj = _auto_resolve("evaluate_point")
        assert obj is not None, (
            f"{_MODULE} 中解析不到 evaluate_point：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.evaluate_point 不可调用 (owner={owner!r})"

    def test_remove_rule_callable(self):
        owner, obj = _auto_resolve("remove_rule")
        assert obj is not None, (
            f"{_MODULE} 中解析不到 remove_rule：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.remove_rule 不可调用 (owner={owner!r})"

    def test_update_rule_callable(self):
        owner, obj = _auto_resolve("update_rule")
        assert obj is not None, (
            f"{_MODULE} 中解析不到 update_rule：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.update_rule 不可调用 (owner={owner!r})"

    def test_get_active_alarms_callable(self):
        owner, obj = _auto_resolve("get_active_alarms")
        assert obj is not None, (
            f"{_MODULE} 中解析不到 get_active_alarms：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.get_active_alarms 不可调用 (owner={owner!r})"

    def test_get_alarm_history_callable(self):
        owner, obj = _auto_resolve("get_alarm_history")
        assert obj is not None, (
            f"{_MODULE} 中解析不到 get_alarm_history：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.get_alarm_history 不可调用 (owner={owner!r})"
