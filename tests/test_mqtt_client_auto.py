...
class TestAutoSmoke:
    ...
    def test_maxlen_callable(self):
        ...
        assert obj is not None, (
            f'{_MODULE} 中解析不到 maxlen：模块属性与所有类属性表均未命中，'
            f'请确认 API 是否已删除或改名')
        assert callable(obj) or isinstance(obj, property), (
            f'{_MODULE}.maxlen 不可调用 (owner={owner!r})')
    ...
    def test_appendleft_callable(self):
        ...
        assert obj is not None, (
            f'{_MODULE} 中解析不到 appendleft：模块属性与所有类属性表均未命中，'
            f'请确认 API 是否已删除或改名')
        assert callable(obj) or isinstance(obj, property), (
            f'{_MODULE}.appendleft 不可调用 (owner={owner!r})')

    def test_popleft_callable(self):
        owner, obj = _auto_resolve('popleft')
        assert obj is not None, (
            f'{_MODULE} 中解析不到 popleft：模块属性与所有类属性表均未命中，'
            f'请确认 API 是否已删除或改名')
        assert callable(obj) or isinstance(obj, property), (
            f'{_MODULE}.popleft 不可调用 (owner={owner!r})')
    ...
    def test_update_will_callable(self):
        ...
        assert obj is not None, (
            f'{_MODULE} 中解析不到 update_will：模块属性与所有类属性表均未命中，'
            f'请确认 API 是否已删除或改名')
        assert callable(obj) or isinstance(obj, property), (
            f'{_MODULE}.update_will 不可调用 (owner={owner!r})')
