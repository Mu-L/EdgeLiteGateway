...
class TestAutoSmoke:
    ...
    def test_set_slave_state_callable(self):
        ...
        assert obj is not None, (
            f"{_MODULE} 中解析不到 set_slave_state：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.set_slave_state 不可调用 (owner={owner!r})"

    def test_read_pdo_callable(self):
        owner, obj = _auto_resolve("read_pdo")
        assert obj is not None, (
            f"{_MODULE} 中解析不到 read_pdo：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.read_pdo 不可调用 (owner={owner!r})"

    def test_write_pdo_callable(self):
        owner, obj = _auto_resolve("write_pdo")
        assert obj is not None, (
            f"{_MODULE} 中解析不到 write_pdo：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.write_pdo 不可调用 (owner={owner!r})"
    ...
    def test_request_dc_sync_callable(self):
        ...
        assert obj is not None, (
            f"{_MODULE} 中解析不到 request_dc_sync：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.request_dc_sync 不可调用 (owner={owner!r})"
