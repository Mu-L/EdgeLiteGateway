"""自动生成冒烟测试 - src.edgelite.drivers.s7_ota
...
_MODULE = "src.edgelite.drivers.s7_ota"
...
class TestAutoSmoke:
    ...
    def test_set_current_version_callable(self):
        owner, obj = _auto_resolve("set_current_version")
        assert obj is not None, (
            f"{_MODULE} 中解析不到 set_current_version：模块属性与所有类属性表均未命中，请确认 API 是否已删除或改名"
        )
        assert callable(obj) or isinstance(obj, property), f"{_MODULE}.set_current_version 不可调用 (owner={owner!r})"
